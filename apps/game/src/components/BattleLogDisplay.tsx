import { type BattleEvent, type PlayerState } from "@webgame/types";
  ...
  const getEventText = (event: BattleEvent, index: number) => {
    ...
    switch (event.type) {
      case "ATTACK": {
        ...
        return (
          <LogItem key={itemKey}>
            <ActorName $teamId={actor?.teamId}>{actor?.id}</ActorName> uses <WeaponName>{weaponName}</WeaponName>{" "}
            against <TargetName $teamId={target?.teamId}>{target?.id}</TargetName>
          </LogItem>
        );
      }
      case "DAMAGE": {
