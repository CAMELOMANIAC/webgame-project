...
- **`src/utils/`**: 전투 시뮬레이션(`simulateBattle.ts`), 계산 로직(`calc.ts`), 커스텀 훅(`useBattlePlayer.ts`) 등 유틸리티 함수 모음.

## 3. 코드 규칙
- **명명 규칙 (Naming Convention)**
  - **파일 및 컴포넌트**: React 컴포넌트 파일 및 클래스는 `PascalCase`를 사용합니다. (예: `BattlePlayer.tsx`)
  - **일반 코드**: 변수, 함수, Atom, 유틸리티 파일 등은 `camelCase`를 사용합니다. (예: `globalAtom.ts`, `simulateBattle.ts`)
...
- **로직 분리**: 복잡한 게임 로직은 `src/utils`에 별도 함수로 분리하고, React 컴포넌트에서는 Jotai Atom을 통해 반응형으로 UI를 업데이트합니다.
